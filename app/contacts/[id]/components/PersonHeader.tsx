...
export function PersonHeader({ contact, onEdit, onToggleFavorite, onAvatarClick }: PersonHeaderProps) {
  ...
  const handleLogInteraction = async (type: 'connection' | 'attempt') => {
      ...
      setIsLogging(true);
      try {
          // Map button types to InteractionType
          const interactionType = type === 'connection' ? 'in-person' : 'call';

          console.log('[CLIENT] Calling logInteraction with type:', interactionType);
          const result = await logInteraction({
              personId: contact.id,
              type: interactionType,
              note: quickNote || undefined
          });
          console.log('[CLIENT] logInteraction result:', result);
