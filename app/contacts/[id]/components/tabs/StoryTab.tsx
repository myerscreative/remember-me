...
export function StoryTab({ contact }: StoryTabProps) {
  ...
  const handleSaveLore = async () => {
    ...
    if (res.success) {
      toast.success("Deep Lore updated! 📚");
    } else {
      toast.error(res.error || "Failed to save lore");
    }
  };
  ...
  const handleAddMilestone = async () => {
    ...
    if (res.success) {
      ...
      // The parent will revalidate path, so we might need a local refresh if not handled by Next
    } else {
        ...
        <div className="flex items-center justify-between mb-2">
           ...
           <div className="flex items-center gap-2">
             ...
             <Button 
               ...
               className="h-8 bg-amber-500 hover:bg-amber-600 text-[#0F172A] font-black uppercase text-[10px] rounded-none px-4"
             >
