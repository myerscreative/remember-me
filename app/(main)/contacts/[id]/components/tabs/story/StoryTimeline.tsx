...
export function StoryTimeline({ items }: StoryTimelineProps) {
  if (items.length === 0) {
    return (
      <div className="text-center py-12 bg-canvas rounded-2xl border border-dashed border-border-default">
        <Sparkles className="h-8 w-8 text-text-tertiary mx-auto mb-3" />
        <p className="text-text-secondary mb-1">No story entries yet</p>
        ...
        {items.map((item) => (
          <div key={item.id} className="relative flex gap-4">
            ...
            <div className="flex-1 min-w-0 pb-2">
              ...
              {item.note && (
                <blockquote className="mt-2 pl-4 border-l-2 border-indigo-200 dark:border-indigo-800 italic text-text-secondary font-serif text-sm">
              ...
              {item.type === 'fact' && item.category && (
                <span className="inline-block mt-1 text-xs px-2 py-0.5 rounded-full bg-subtle text-text-secondary">
                  {item.category}
                </span>
              )}
            </div>
