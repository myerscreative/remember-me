...
export const EntitySuggestionBar = ({ suggestions, onConfirm }: EntitySuggestionBarProps) => {
  ...
  return (
    <div className="flex flex-col gap-2 py-3 border-t border-border-default mt-2 animate-in slide-in-from-bottom-2 fade-in duration-300">
      ...
      <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
        {suggestions.map((s) => (
          <button
            key={s.id}
            onClick={() => onConfirm(s)}
            className="flex items-center gap-2 bg-canvas border border-dashed border-border-default hover:border-indigo-500 hover:bg-indigo-500/10 px-3 py-2 rounded-xl transition-all shrink-0 group"
          >
            ...
            <span className="text-xs text-text-primary group-hover:text-text-primary">{s.label}</span>
          </button>
        ))}
      </div>
    </div>
