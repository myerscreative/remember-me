  ...
  return (
    <div className="min-h-screen bg-canvas p-8">
      <div className="max-w-4xl mx-auto space-y-6">
        <header className="flex items-center justify-between pb-6 border-b border-border-default">
          <button 
            onClick={() => router.back()}
          ...
          <div className="text-right">
             <h1 className="text-2xl font-bold text-text-primary">Network Cleanup</h1>
             <p className="text-xs text-text-secondary">AI-Powered Duplicate Detection</p>
          </div>
        </header>

        {duplicateGroups.length === 0 ? (
          <div className="text-center py-20 bg-surface rounded-xl shadow-sm border border-border-default">
            <div className="w-16 h-16 bg-green-100 text-green-600 rounded-full flex items-center justify-center mx-auto mb-4">
              <Check className="w-8 h-8" />
            </div>
            <h2 className="text-xl font-semibold text-text-primary mb-2">No Duplicates Found</h2>
            <p className="text-text-secondary">Your network looks clean! We checked names, emails, and phone numbers.</p>
          </div>
        ) : (
          <div className="space-y-6">
            ...
            {duplicateGroups.map((group) => (
              <div key={group.id} className="bg-surface rounded-xl shadow-sm border border-border-default overflow-hidden">
                <div className="p-4 bg-canvas border-b border-border-default flex justify-between items-center">
                  <div>
                    ...
                    <div className="text-xs text-text-secondary mt-1 flex gap-2">
                      {group.reason.map((r, i) => (
                        <span key={i} className="bg-border-default px-1.5 py-0.5 rounded">{r}</span>
                      ))}
                    </div>
                  </div>
                ...
                <div className="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    {[group.keeper, ...group.duplicates].slice(0, 3).map(person => (
                       <div key={person.id} className="flex items-center gap-3 p-2 bg-canvas rounded-lg border border-border-default">
                           <div className="w-8 h-8 rounded-full bg-border-default flex items-center justify-center text-xs font-bold text-text-tertiary overflow-hidden">
                           ...
                           <div className="overflow-hidden">
                              ...
                              <div className="text-xs text-text-secondary truncate">{person.email || person.phone || 'No info'}</div>
                           </div>
                       </div>
                    ))}
                    {group.duplicates.length + 1 > 3 && (
                       <div className="flex items-center justify-center text-xs text-text-secondary italic">
