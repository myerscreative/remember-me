  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-[800px] mx-auto min-h-screen md:min-h-0 md:my-8 bg-canvas md:rounded-3xl shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="hidden md:block">
          ...
          <div className="px-8 py-8 bg-linear-to-br from-blue-50 to-blue-100  border-b-2 border-indigo-500 flex gap-6 items-center">
            ...
            <div className="flex-1">
              <h2 className="text-3xl font-bold text-text-primary mb-3">Meeting with {contact.name}</h2>
            ...
            <div className="grid gap-6">
              ...
              <div className="flex gap-4 p-5 bg-canvas rounded-2xl border border-border-default">
                ...
                <div>
                  ...
                  <p className="text-base text-text-primary mb-1 font-medium">{contact.whereWeMet} - {contact.whenWeMet}</p>
                  <p className="text-sm text-text-secondary italic">{contact.howWeMet}</p>
                </div>
              </div>
              ...
              <div className="flex gap-4 p-5 bg-canvas rounded-2xl border border-border-default">
                <div className="text-2xl mt-1">💭</div>
                <div>
                  <h4 className="text-sm font-semibold text-text-secondary uppercase tracking-wider mb-2">What We Talked About</h4>
                  <ul className="list-disc pl-4 space-y-1.5 text-text-tertiary">
              ...
              <div className="flex gap-4 p-5 bg-canvas rounded-2xl border border-border-default">
                ...
                <div>
                  ...
                  <p className="text-text-primary">{contact.whyStayInContact}</p>
                </div>
              </div>
          ...
          <div className="px-8 py-8 border-b border-border-default">
            ...
            <div className="space-y-4">
              {meeting.mutualConnections.map((conn) => (
                <div key={conn.id} className="p-5 bg-canvas border-2 border-border-default rounded-2xl hover:bg-surface hover:border-indigo-500 transition-colors group">
                  <div className="flex items-center gap-4 mb-4">
                    <div className="w-12 h-12 rounded-full bg-purple-500 text-white flex items-center justify-center font-bold">
                      {conn.initials}
                    </div>
                    <div>
                      <h4 className="font-bold text-text-primary text-lg">{conn.name}</h4>
                      <p className="text-text-secondary">{conn.role}</p>
                    </div>
                  </div>
                  <div className="mb-4">
        ...
        <div className="md:hidden flex-1 overflow-x-auto snap-x snap-mandatory flex bg-subtle pb-8 pt-4 px-4 gap-4 no-scrollbar">
          ...
          <div className="min-w-[calc(100vw-2rem)] snap-start bg-canvas rounded-3xl p-6 shadow-xl flex flex-col items-center text-center overflow-y-auto max-h-[calc(100vh-180px)]">
            ...
            <h2 className="text-2xl font-bold text-text-primary mb-1">{contact.name}</h2>
            <p className="text-text-secondary mb-6 font-medium">{contact.role}</p>
            ...
            {meeting.locationUrl ? (
              ...
              <div className="w-full py-4 bg-subtle text-text-secondary rounded-2xl font-bold mb-4 flex items-center justify-center gap-2">
                <MapPin className="w-5 h-5" /> {meeting.location}
              </div>
            )}

            <div className="mt-auto pt-4 text-text-secondary text-sm font-medium animate-pulse">
              ← Swipe for context →
            </div>
          </div>

          {/* Card 2: Key Context */}
          <div className="min-w-[calc(100vw-2rem)] snap-start bg-canvas rounded-3xl p-6 shadow-xl flex flex-col items-center overflow-y-auto max-h-[calc(100vh-180px)]">
             <div className="text-xs font-bold text-text-secondary uppercase tracking-widest mb-6 border-b border-border-default pb-2 w-full text-center">Key Context</div>

             <div className="w-full space-y-4">
               {/* Last Spoke */}
               <div className="p-4 bg-canvas rounded-2xl border border-border-default flex gap-4 items-start text-left">
                  <span className="text-2xl">💭</span>
                  <div>
                    <strong className="block text-text-secondary text-xs uppercase mb-1">Last Spoke</strong>
                    <p className="text-text-primary font-medium">
                      {contact.lastContact.daysAgo ? `${contact.lastContact.daysAgo} days ago` : 'Never'}
                    </p>
                    <p className="text-sm text-text-secondary mt-1">"{contact.lastContact.notes}"</p>
                  </div>
               </div>
               ...
               {contact.whatMattersToThem[0] && (
                 <div className="p-4 bg-canvas rounded-2xl border border-border-default flex gap-4 items-start text-left">
                    <span className="text-2xl">👨‍👩‍👧</span>
                    <div>
                      <strong className="block text-text-secondary text-xs uppercase mb-1">What Matters</strong>
                      <p className="text-text-primary font-medium">{contact.whatMattersToThem[0]}</p>
                    </div>
                 </div>
               )}
               ...
               {meeting.mutualConnections[0]?.sharedInterests[0] && (
                 <div className="p-4 bg-canvas rounded-2xl border border-border-default flex gap-4 items-start text-left">
                    <span className="text-2xl">🎣</span>
                    <div>
                      <strong className="block text-text-secondary text-xs uppercase mb-1">Shared Interest</strong>
                      <p className="text-text-primary font-medium">
             ...
             <div className="mt-auto pt-8 text-text-secondary text-sm font-medium">
              ← Swipe for more →
             </div>
          </div>
          ...
          <div className="min-w-[calc(100vw-2rem)] snap-start bg-canvas rounded-3xl p-6 shadow-xl flex flex-col items-center overflow-y-auto max-h-[calc(100vh-180px)]">
            <div className="text-xs font-bold text-text-secondary uppercase tracking-widest mb-6 border-b border-border-default pb-2 w-full text-center">Say This</div>
        ...
        <div className="md:hidden absolute bottom-6 left-0 right-0 flex justify-center gap-2 pointer-events-none">
           ...
           <div className="w-2 h-2 bg-border-strong rounded-full"></div>
           <div className="w-2 h-2 bg-border-strong rounded-full"></div>
        </div>
