...
export function FactMatchGame({ onBack }: FactMatchGameProps) {
  ...
  if (contacts.length < 4) {
      return (
        <div className="flex flex-col items-center justify-center min-h-[50vh] p-8 text-center">
            <p className="text-xl text-text-secondary mb-4">Not enough contacts to play.</p>
  ...
  return (
    ...
    >
      {/* Game Card */}
      <div className="bg-surface rounded-3xl shadow-xl overflow-hidden border border-border-default">

        {/* Question Header */}
        <div className="bg-linear-to-br from-subtle to-subtle p-8 md:p-12 flex flex-col items-center text-center relative overflow-hidden">
             ...
             <div className="bg-surface p-6 rounded-full shadow-lg mb-6 relative z-10">
                 {renderIcon()}
             </div>
             <h2 className="text-2xl md:text-3xl font-bold text-text-primary leading-tight relative z-10 max-w-lg">
                {renderPrompt()}
             </h2>
        </div>
        ...
        <div className="p-6 md:p-8">
            <div className="grid grid-cols-1 gap-3">
                {gameState.question.options.map((option) => {
                    ...
                    let buttonStyle = "bg-surface border-2 border-border-default hover:border-pink-300 hover:bg-pink-50 text-text-primary";
                    ...
                    if (gameState.selectedOption) {
                        if (isTarget) {
                            ...
                            buttonStyle = "bg-surface border-border-default text-text-tertiary opacity-50";
                        }
                    }
                    ...
                    return (
                        ...
                        >
                            <span className="flex items-center gap-3">
                                {option.photo ? (
                                    <img src={option.photo} alt={option.name} className="w-10 h-10 rounded-full object-cover border border-border-default" />
                                ) : (
                                    <div className="w-10 h-10 rounded-full bg-subtle flex items-center justify-center text-text-tertiary text-xs font-bold">
                                        {option.initials}
                                    </div>
                                )}
        ...
        {gameState.selectedOption && (
            <div className="p-4 bg-surface border-t border-border-default flex justify-center animate-in slide-in-from-bottom-4 fade-in">
                <Button 
